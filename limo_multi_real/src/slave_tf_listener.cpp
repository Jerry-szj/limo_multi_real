...
double max_vel_x=2.0;     //最大速度限制
double min_vel_x=0.05;     //最小速度限制
double max_vel_theta=2.0; //最大角速度限制
double min_vel_theta=0.05;//最小角速度限制

double k_v=1;   //调节前后方向偏差时，k_v越大，线速度越大
double k_l=10;   //调节左右方向偏差时，k_l越大，角速度越大
double k_a=100;   //调节角度偏差时，k_a越大，角速度越大
...
int main(int argc, char** argv){
  ...
  while (node.ok()){
    ...
    else if(multi_mode==2)
    {
      ...
      double angle_diff = trans_z - angular_z2; // 计算原始角度差
      ...
      e_angular_z = atan2(sin(angle_diff), cos(angle_diff));       
                                               //角度偏差
      ROS_INFO("Deviation: (e_z =%f)", e_angular_z);
